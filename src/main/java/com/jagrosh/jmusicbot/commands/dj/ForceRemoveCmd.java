...
public class ForceRemoveCmd extends DJCommand
{
    public ForceRemoveCmd(Bot bot)
    {
        ...
        this.help = "Elimina todas las entradas de un usuario en la cola";
        this.arguments = "<user>";
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        {
            event.replyError("¡Tienes que mencionar a un usuario!");
            return;
        }
        ...
        if(found.isEmpty())
        {
            event.replyError("¡No se puede encontrar el usuario!");
            return;
        }
        else if(found.size()>1)
        {
            ...
            .setText("Se han encontrado múltiples usuarios:")
    ...
    private void removeAllEntries(User target, CommandEvent event)
    {
        ...
        if (count == 0)
        {
            event.replyWarning("¡**"+target.getName()+"** no tiene canciones en la cola!");
        }
        else
        {
            event.replySuccess("Se han eliminado `"+count+"` entradas de **"+target.getName()+"**#"+target.getDiscriminator()+".");
        }
    }
}
